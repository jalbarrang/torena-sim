...
- **pinned**: skills the game fired are forced at the recorded distance with `forced_positions`, skills it never fired are removed, the last spurt starts at the recorded distance through `CreateRunner::forced_last_spurt_distance`, and rushed spells run where the game recorded them through `forced_rushed_regions` with the engine's own rushed roll disabled. What remains is deterministic (speed, acceleration, HP) plus the rolls the replay never records, so a pinned residual is either a formula error or one of those rolls.
...
Outcomes the replay does not record and the engine still rolls in pinned mode: per-section wit variance, downhill mode, and dueling. Lane position and the blocking runner are recorded but have no seam yet. The recorded spurt distance pins the transition only; the spurt speed follows from the HP formula for that transition.
