...
export function RunnerAvatar(props: RunnerAvatarProps) {
  const { runner, className } = props;

  return (
    <span
      className={cn(
        'relative inline-flex size-9 shrink-0 items-center justify-center overflow-hidden rounded-full',
        className
      )}
    >
      <img
        ...
        {runners.map((runner) => {
          ...
          const identity = (
            <button
              type="button"
              onClick={() =>
                picking ? handleAssign(runner.fieldId, pickRole) : handleEdit(runner.fieldId)
              }
